...
After synchronization is enabled, every supported row mutation atomically records an ordered SQLite changeset and operation context alongside the row changes. A local transaction is durable only according to the local machine until a push confirms a new remote head.
...
| ---------------------------------------------------------------- | ----------------------------------------------------------- |
| Transactions affect different rows and still satisfy constraints | Silo rebases the pending transactions in order.             |
| Transactions incompatibly affect the same row                    | Pull or push stops and preserves the active local database. |
