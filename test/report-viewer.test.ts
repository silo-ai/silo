...
describe('report viewer', () => {
  ...
  test('serves stale-first HTML and protects focus-triggered refreshes', async () => {
    ...
    expect(html).toContain('data-report-toc-container')
    expect(html).toContain('aria-controls="report-toc-menu"')
    expect(html).toContain('class="report-menu-icon"')
    expect(html).toContain('buildTableOfContents')
    expect(html).toContain("querySelectorAll('h2, h3, h4, h5')")
    expect(html).toContain('data-report-toc-corridor')
    expect(html).toContain('createTocSubmenu')
    expect(html).toContain('setTocSubmenuVerticalPlacement')
    expect(html).toContain('setTocMenuWidth')
    expect(html).toContain('siloReportViewerPretext')
    ...
    expect(cssText).toContain('.report-toolbar')
    expect(cssText).toContain('position: sticky')
    expect(cssText).toContain('.report-toc:hover .report-toc-menu')
    expect(cssText).toContain('.report-toc::after')
    expect(cssText).toContain('.report-toc-submenu')
    expect(cssText).toContain('.report-toc-corridor')
    expect(cssText).toContain('pointer-events: auto')
    expect(cssText).toContain('overflow: visible')
    expect(cssText).toContain('left: calc(100% - 0.35rem)')
    expect(cssText).toContain('right: calc(100% - 0.35rem)')
    expect(cssText).toContain('max-height: min(60vh, 32rem, calc(100vh - 1rem))')
    expect(cssText).toContain('background: var(--background)')
    expect(cssText).toContain('box-shadow: 0 0.75rem 2rem rgba(0, 0, 0, 0.08)')
